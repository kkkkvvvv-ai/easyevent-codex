...
impl Session {
    pub(crate) async fn switch_model_provider(
        ...
        let _guard = acquire_persistence_lock(self).await;
        let commit = self
            .update_settings(updates)
            .await
            .map_err(|error| codex_protocol::error::CodexErr::InvalidRequest(error.to_string()))?;
        if let Some(live) = self.live_thread() {
            live.append_items(&[RolloutItem::EventMsg(applied_event(self).await)])
                .await
                .map_err(|error| {
                    codex_protocol::error::CodexErr::InvalidRequest(error.to_string())
                })?;
            live.flush().await.map_err(|error| {
                codex_protocol::error::CodexErr::InvalidRequest(error.to_string())
            })?;
        }
        emit_applied(self, super::new_submission_id(), commit.snapshot).await;
        Ok(())
    }
